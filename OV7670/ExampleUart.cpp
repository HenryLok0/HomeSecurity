...
//
// set EXAMPLE to 3 (UART example) in setup.h to activate
#include "setup.h"
#if EXAMPLE == 3
...
//
// 推荐值：9  →  320x240, 1Mbps, RGB
#define UART_MODE 2


const uint8_t VERSION = 0x20; // Changed from 0x10 to 0x20 to avoid XON (0x11) control character
const uint8_t COMMAND_NEW_FRAME = 0x01 | VERSION; // Now 0x21
const uint8_t COMMAND_DEBUG_DATA = 0x03 | VERSION; // Now 0x23
...
void initializeScreenAndCamera() {
  ...
  // 可选：如果是 WAVGAT 之类兼容板才需要下面两行
  //CLKPR = 0x80; // enter clock rate change mode
  //CLKPR = 1;    // set prescaler to 1

  Serial.begin(baud);
  delay(2000);   // 等 PC 那边把串口准备好

  // 调试：开始初始化摄像头
  commandDebugPrint("INIT START");

  bool ok = camera.init();

  if (ok) {
    commandDebugPrint("INIT OK");
    // 先发一帧绿色，表示摄像头初始化成功
    sendBlankFrame(COLOR_GREEN);
  } else {
    commandDebugPrint("INIT FAIL");
    // 发一帧红色，表示初始化失败（接线/摄像头问题）
...
void commandStartNewFrame(uint8_t pixelFormat) {
  ...
  checksum = sendNextCommandByte(checksum, 
      ((lineLength >> 8) & 0x03)     // higher 2 bits of width
      | ((lineCount >> 6) & 0x0C)    // higher 2 bits of height
      | ((pixelFormat << 4) & 0xF0));
